...
namespace actkin_balancer {
  ...
  protected:
    ...
    public:
      ...
      double doubleTime = 0.0; // はじめdoubleTimeだけ両足支持で待つ. supportLeg==NUM_LEGSなら使用しない.
    ...
    std::vector<std::vector<double> > sampledTheta=std::vector<std::vector<double> >(NUM_LEGS); // swingLegがrleg/lleg. 支持脚相対
